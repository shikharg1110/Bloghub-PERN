...
const MyBlog = () => {
    ...
    const fetchmyBlog = async ( query = '', tag = null, pageNumber = 1, user) => {
        setLoading(true);
        try {
            const response = await axios.get(`${import.meta.env.VITE_SERVER_DOMAIN}/myBlog`, {
                params: { query, tag, page: pageNumber, limit: 6 , user}
            });
    ...
    useEffect(() => {
        // setBlogs([]);
        setPage(1);
        fetchmyBlog(searchQuery, selectedTag, 1, user);
    }, [searchQuery, selectedTag]);

    useEffect(() => {
        if(page > 1) {
            fetchmyBlog(searchQuery,selectedTag, page, user);
        }
    }, [page, selectedTag]);
